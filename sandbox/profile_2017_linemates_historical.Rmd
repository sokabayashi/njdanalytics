...
# player_name <- "Erik Karlsson" # 8474578 BUG.  there are two Erik Karlsson
# player_name <- "Brent Burns" # 8474578 BUG.  there are two Erik Karlsson
# player_name <- "Nail Yakupov"
# player_name <- "Colin Wilson"
# player_name <- "Jaromir Jagr"
# player_name <- "Mikkel Boedker"
# player_name <- "Jordan Schroeder"
...
season_h2h  <- tbl( nhl_db, "season_h2h_ev5on5"       )
```
...
player_h2h <- season_h2h %>% filter( nhl_id_1 == player_bio$nhl_id ) %>% select(-id)

################
#### AGE #######
################
# age at start of 2016-17 season.  This will need to be updated depending on report run date
player_age_915 <- player_bio$age_season_start
...
earliest_season_db <- "20112012"
player_gp  <- player_gp  %>% filter( season >= earliest_season_db )
player_h2h <- player_h2h %>% filter( season >= earliest_season_db )
...
this_player_seasons_sh <- player_gp %>% 
                  filter( session_id==this_session_id, filter_strength=="sh" ) %>% 
                  group_by( season ) %>% 
                  summarize(
                    gm  = n(),
                    toi = sum(toi)
                  ...
                  mutate(
                    toi_gm_sh       = (toi/gm) %>% round(1)
                  ) %>% ungroup() %>% arrange( season ) %>% select( -(gm:toi) )
this_player_seasons_pp <- player_gp %>% 
                  filter( session_id==this_session_id, filter_strength=="pp" ) %>% 
                  group_by( season ) %>% 
                  summarize(
                    gm  = n(),
                    toi = sum(toi)
                  ...
                  mutate(
                    toi_gm_pp       = (toi/gm) %>% round(1)
                  ) %>% ungroup() %>% arrange( season ) %>% select( -(gm:toi) )
this_player_seasons <- this_player_seasons_all %>% 
                        left_join( this_player_seasons_pp, by="season" ) %>% 
                        left_join( this_player_seasons_sh, by="season" )

# toi season melted
toi_special_by_season <- this_player_seasons %>% select( season, season_end, season_age_label, toi_gm_pp, toi_gm_sh ) %>% 
                            gather( strength, toi_gm, toi_gm_pp:toi_gm_sh )


# earliest season--within game_player, not player's history)
min_season_end    <- min( this_player_seasons$season_end, na.rm=T )
recent_season_end <- max( this_player_seasons$season_end )

# chart only last __ seasons
num_seasons_to_chart <- 5
...
seasons_played    <- c( player_hr$season_end, this_player_seasons$season_end ) %>% unique() %>% na.omit() %>% length()

player_gp           <- player_gp %>% filter( season >= min_season_db )
this_player_seasons <- this_player_seasons %>% filter( season >= min_season_db )
...
line_chart_theme <- theme_bw() + theme(
        ...
        plot.margin = unit( c(0.3, 0.2, 0, 0.4), units="lines"), # top right bottoom left
...
comparison_vars <- read_csv( file=paste0(nhl_dir$db, "/player_comparison_cols.csv"), na="", trim_ws=F )
comparison_vars$format_pct[is.na(comparison_vars$format_pct) ] <-""

drop_cols <- c( ) #"sf_i_gm_ev5on5" )
comparison_vars <- comparison_vars %>% filter( !comp_col %in% drop_cols )
...
this_player_season <- this_player_season %>% left_join( this_player_seasons %>% select( season_end, season_age_label ),
                                          by="season_end" ) 
```

```{r this player linemates by season}
this_player_linemates <- player_h2h %>% 
                          filter( session_id=="2", team_comp=="T", position_fd_2==player_bio$position_fd, last_name_2 != last_name_1 ) %>% 
                          collect()

top_linemate_season <- this_player_linemates %>%
                        group_by( season, nhl_id_1, last_name_1 ) %>%
                        arrange( season, desc(toi_ev5on5) ) %>% top_n(n=4, wt=toi_ev5on5 )

top_linemate_season_m <- top_linemate_season %>% ungroup() %>% 
                          group_by( season ) %>%
                          arrange( season, toi_pct ) %>%
                          mutate(
                            toi_season_pct = round( toi_pct/sum(toi_pct)*100, 1 ),
                            # cumsum_toi_pct   = cumsum(toi_season_pct ),
                            toi_rank       = row_number(),
                            toi_label      = paste0( last_name_2, "  ", round(toi_pct), "%" )
)
top_linemate_season_m <- top_linemate_season_m %>% left_join( this_player_seasons %>% select( season, season_end, season_age_label ),
                                          by="season" ) 
```
...
player_teams <- player_teams %>% mutate(
  ...
  last_season_xtext      = season_end
)

if( num_team_seasons > 1 ) {
  # first team, right justify to team change line
  player_teams$last_season_xtext[ 1] <- player_teams$last_season_xtext[ 1 ] + 0.40 
  # all others, put to just left of team change line
  player_teams$last_season_xtext[-1] <- player_teams$last_season_xtext[-num_team_seasons] + 0.75 # trial and error

  # if game numbers (multiple teams in a season), then push to right a bit more  
  player_teams <- player_teams %>% ungroup() %>% mutate(
    last_season_xtext = ifelse( row_number(season) !=1 & str_detect( team_short, "\n"),
                                season_end - 0.08, last_season_xtext )
  )
}

if( num_team_seasons > 1 ) {
  team_lines <- geom_vline( data=player_teams[-num_team_seasons,],
                            aes(xintercept=last_season_xintercept),color="orange",alpha=0.6, size=0.5)
  # hjust=1.
  team_texts <- geom_text(  data=player_teams, aes(x=last_season_xtext, y=team_y_min, label=team_short ), 
                            color="slateblue", hjust=1, vjust= 0, size=1.5 )
...
```{r career GP,fig.width=2.7, fig.height=1.1}
team_y_min <- 0
y_gp_min   <- ( min( this_player_seasons$gm )*.9 ) %>% round()
y_gp_max   <- ( max( this_player_seasons$gm )*1.2 ) %>% round()
team_y_min <- y_gp_min

p.gp <- ggplot( this_player_seasons, aes(x=season_end, y=gm) )
p.gp + 
  team_lines + 
  geom_line(  color=gp_color, size=career_line_size ) +
  geom_point( color=gp_color, size=career_point_size ) +
  team_texts + 
  geom_text( aes(label=gm), vjust=-1, size=geom_text_size ) +
  scale_y_continuous( "Games Played", breaks=seq(0, 120, 20), limits=c(y_gp_min, y_gp_max) ) +
  scale_x_continuous( breaks=this_player_seasons$season_end, labels=this_player_seasons$season_age_label ) + 
  line_chart_theme + scale_color_few() +
  ggtitle( "Games Played" ) 
```
```{r career linemates,fig.width=5.7, fig.height=1.1}
title_linemate <- sprintf( "Top Linemates (Pct of %s's Total 5v5 TOI)", player_bio$last_name ) 

p.linemate_season <- ggplot( top_linemate_season_m, aes(x=season_end, y=toi_rank) )
p.linemate_season +
  geom_tile( aes( fill=toi_pct ), color="white", size=0.3 ) +
  scale_fill_gradient( low="white", high=few_blue, guide=FALSE, space="Lab" ) +
  geom_text( aes(x=season_end+0.3, y=toi_rank, label=toi_label), size=geom_text_size, vjust = 0.4, hjust = 1 ) + 
  # scale_y_continuous( "", breaks=seq(0, 120, 20), limits=c(0, 100) ) +
  scale_x_continuous( breaks=this_player_seasons$season_end, labels=this_player_seasons$season_age_label, expand = c(0, 0) ) + 
  line_chart_theme +
  theme( 
    axis.text.y  = element_blank(),
    axis.ticks   = element_blank(),
    # plot.margin = unit( c(0.3, 0.2, 0, 0.4), units="lines"), # from line_chart_theme
    plot.margin = unit( c(0.3, 2, 0, 0.4), units="lines") # top right bottom left
    # panel.margin = unit(0,"null")
    ) +
  ggtitle( title_linemate ) 
...
```{r career TOI per Gm, fig.width=2.7, fig.height=1.1}
y_lim_min_toi_gm <- min( this_player_seasons$toi_gm )*.9
y_lim_max_toi_gm <- max( this_player_seasons$toi_gm )*1.1
team_y_min <- y_lim_min_toi_gm

p.toi <- ggplot( this_player_seasons, aes(x=season_end, y=toi_gm) )
p.toi + 
  geom_hline( yintercept=c(15,20), size=0.5, color="lightblue" ) + 
  team_lines + 
  geom_line(  color=toi_color, size=career_line_size ) +
  geom_point( color=toi_color, size=career_point_size ) +
  team_texts + 
  geom_text( aes(label=round(toi_gm,1) ), vjust=-1, size=geom_text_size ) +
  scale_y_continuous( "", breaks=seq( 0, 35, 2),  limits=c(y_lim_min_toi_gm,y_lim_max_toi_gm) ) +  
  scale_x_continuous( breaks=this_player_seasons$season_end, labels=this_player_seasons$season_age_label) + 
  line_chart_theme + scale_color_few() +
  ggtitle( "TOI/Game (All Strength)" ) 
```
```{r career cf by season,fig.width=2.7, fig.height=1.1}
...
```
```{r career Pts per Game,fig.width=2.7, fig.height=1.1}
y_lim_min_pts_gm <- min( this_player_seasons$pts_gm ) * 0.8
y_lim_max_pts_gm <- max( this_player_seasons$pts_gm ) * 1.3
team_y_min <- y_lim_min_pts_gm
min_max_diff_pts_gm <- y_lim_max_pts_gm - y_lim_min_pts_gm
y_break_step <- 0.1
if( min_max_diff_pts_gm > 0.6 ) y_break_step <- 0.2

p.pts_gm <- ggplot( this_player_seasons, aes(x=season_end, y=pts_gm) )
p.pts_gm +
  team_lines + 
  geom_line(  color=pts_color, size=career_line_size ) +
  geom_point( color=pts_color, size=career_point_size ) +
  team_texts +
  geom_text( aes(label=round(pts_gm,2) ), vjust=-1, size=geom_text_size ) +
  scale_y_continuous( "", breaks=seq( 0, 5, y_break_step), limits=c(y_lim_min_pts_gm,y_lim_max_pts_gm) ) +
  scale_x_continuous( breaks=this_player_seasons$season_end, labels=this_player_seasons$season_age_label) +
  line_chart_theme + scale_color_few() +
  ggtitle( "Points/Game")
```
...
```{r career TOI per Gm PP PK, fig.width=2.7, fig.height=1.1}
y_lim_max_toi_gm_special <- max( 2, toi_special_by_season$toi_gm )*1.8
team_y_min <- 0

p.toi_special <- ggplot( toi_special_by_season, aes(x=season_end, y=toi_gm) )
p.toi_special + 
  team_lines + 
  geom_line(  aes(color=strength ), size=career_line_size ) +
  geom_point( aes(color=strength ), size=career_point_size ) +
  team_texts + 
  geom_text( aes(label=round(toi_gm,1) ), vjust=-1, size=geom_text_size, check_overlap=TRUE ) +
  scale_color_manual( name=NULL,
                     limits=c( "toi_gm_pp", "toi_gm_sh" ),
                     labels=c( "PP", "PK" ),
     guide = guide_legend(reverse=FALSE),
     values=alpha(c( few_darkorange, few_darkred ), 1) ) + 
  scale_y_continuous( "", breaks=seq( 0, 10, 2),  limits=c(0, y_lim_max_toi_gm_special) ) +  
  scale_x_continuous( breaks=toi_special_by_season$season_end, labels=toi_special_by_season$season_age_label) + 
  line_chart_theme + 
  ggtitle( "TOI/Game (Special Teams)" )  + 
  theme(
...
```
```{r career cf rel by season,fig.width=2.7, fig.height=1.1}
# theme_set(theme_gray(base_size = 9))
...
p.cf + 
  geom_hline( yintercept=c(0), size=0.5, color="lightblue" ) + 
  team_lines + 
...
```
```{r career p_60 by season,fig.width=2.7, fig.height=1.1}
# theme_set(theme_gray(base_size = 9))
this_var <- "p_60_ev5on5"

y_lim_min_p_60 <- min( this_player_season[ ,this_var] ) *.8
# y_lim_min_p_60 <- 0

y_lim_max_p_60 <- max( this_player_season[ ,this_var]) *1.3
...
p.stats <- p.stats + 
  geom_bar( stat="identity", aes(fill=category), width=0.65 ) + 
  geom_errorbar( stat="identity", aes(x=variable,y=team_lead_ptile, ymin=team_lead_ptile, ymax=team_lead_ptile), 
                 width=bullet_width_outer, color="darkorange", size=0.85 ) +
    
  scale_y_continuous( "", breaks=seq(0,1,0.1), limits=c(0,1.15), expand = c(0,0), label=percent ) + 
  ...
  geom_text( aes( label= bar_label, y=percentile+0.01 ), hjust= 0, size=2.3, parse=F) + # don't decrease size smaller than 2.3
  # scale_fill_pander() +
  ...
  theme( 
         ...
         plot.margin = unit( c(0.1, 0.2, 0, 0.3), units="lines"), # top right bottom left
         panel.border = element_blank(),
         panel.background = element_blank()
...
if( recent ){
  p.stats <- p.stats + scale_fill_brewer( type="qual", palette=2 )
} else {
  p.stats <- p.stats + scale_fill_few( palette = "dark")
}
...
```{r  main bar recent, fig.width=8, fig.height=2}
this_season <- "20152016"
# this_season <- "20142015"
this_season_gm_min <- 30
recent_season_categories <- c( "toi", "toi_pct", "zs" )
toi_comparison_vars <- comparison_vars %>% filter( category %in% recent_season_categories )

p.toi_comp <- generate_comparison_bullet_chart( this_nhl_id=player_bio$nhl_id, this_season = this_season, player_seasons, player_tbl, toi_comparison_vars, recent=TRUE, gm_requirement=this_season_gm_min )

# p.toi_comp
```

```{r  main bar last 2 years, fig.width=8, fig.height=3}
this_season <- "20142016"
# this_season <- "20132015"
nontoi_comparison_vars <- comparison_vars %>% filter( !category %in% c( "bio", recent_season_categories ) )
p.pts_comp <- generate_comparison_bullet_chart( this_nhl_id=player_bio$nhl_id, this_season = this_season, player_seasons, player_tbl, nontoi_comparison_vars)
...
```{r combined comparison charts, fig.width=8, fig.height=5.}
comparison_bar_charts <- rbind_plots( list( p.toi_comp, 
                                      p.pts_comp ), 
                         c(1, 2 ) )

grid.newpage()
grid.draw(comparison_bar_charts)
```
...
team_games           <- add_team_score_label( team_games ) %>%
  ...
  mutate(
    ...
    date_opponent_win_loss  = paste0( gsub( "@", "", date_opponent ), " ", win_loss )
...
playoff_line_color <- few_green # few_green #"green"
...
p.linemates <- p.linemates + 
  ...
  # geom_text( data=game_dates, aes(label=date_opponent_win_loss, y=-1), color=few_darkblue, angle=90, size=1.3, hjust=0 ) +
  ...
  geom_vline(data=playoff_series, aes(xintercept=player_game_number-0.5), color=playoff_line_color, linetype=1 ) +
...
p.game_result <- ggplot( game_dates, aes(player_game_number, y=0.0) )
p.game_result <- p.game_result + 
  geom_blank() +
  # geom_hline(yintercept=0.2, color="green", size=0.2 ) +
  geom_text( aes(label=date_opponent_result), color=few_blue, angle=90, size=2.5, hjust=0 ) +
  
  geom_vline(xintercept=seq(10.5, max_games_floor_10+0.5, x_grid_interval), color=ten_game_separator_color, size=0.2 ) +
  geom_vline(data=playoff_series, aes(xintercept=player_game_number-0.5), color=playoff_line_color, linetype=1 ) +
  # team change line
  geom_vline(data=team_changes, aes(xintercept=player_game_number-0.5), color="orange" ) + 
  scale_x_continuous(NULL, minor_breaks =NULL, breaks=seq(-0,120,10), expand = c(0, 0)) +
  scale_y_continuous( NULL, limits=c(0,0.01) ) +
  theme_chances
# p.game_result
```
...
p.toi <- p.toi + 
  geom_text( data=game_dates, aes(label=date_opponent_win_loss, y=date_opponent_y), color=few_darkblue, angle=90, size=1.4, hjust=0 ) +
 ...
 scale_fill_manual( name=NULL,
  ...
  geom_hline( yintercept=seq(5,20,5), size=0.5, color="grey" ) +
  ...
  geom_vline(data=team_changes, aes(xintercept=player_game_number-0.5), color="orange" ) + 
  geom_vline(data=playoff_series, aes(xintercept=player_game_number-0.5), color=playoff_line_color, linetype=1 ) +
  scale_y_continuous( "TOI",        minor_breaks=NULL,  breaks=seq(0,40,5),     limits=c(date_opponent_y,toi_max) ) +
...
theme_top    <- theme( plot.margin=unit(c( 0.5, 1, -0.2, 1), "cm"), # bottom previously 0.1
                      axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank() )

theme_middle <- theme( plot.margin=unit(c( -0.2, 1, -0.2, 1), "cm"), # bottom previously 0.1
  axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank() )

theme_bottom <- theme( plot.margin=unit(c(-0.2,1,1,1), "cm"))
...
p.game_result_bottom<- p.game_result + theme_bottom
