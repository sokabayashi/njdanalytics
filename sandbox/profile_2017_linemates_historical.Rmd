...
p.pts_gm +
  ...
  scale_y_continuous( "", breaks=seq( 0, 5, 0.1), limits=c(y_lim_min_pts_gm,y_lim_max_pts_gm) ) +
  scale_x_continuous( breaks=this_player_seasons$season_end, labels=this_player_seasons$season_age_label) +
  line_chart_theme + scale_color_few() +
  ggtitle( "Points/Game")
```
...
if( recent ){
p.stats <- p.stats + 
  geom_bar( stat="identity", fill="cornflowerblue", width=0.65 )  
...
theme_middle <- theme( plot.margin=unit(c( -0.2, 1, -0.1, 1), "cm"), # bottom previously 0.1
  axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank() )
