...
njd_roster <- vs_opp_rosters %>% filter( team_short=="NJD" ) %>% select( nhl_id, number, last_name, position ) %>% 
...
# Larsson
this_nhl_id <- player_tbl %>% filter( first_last_name=="Adam Larsson" ) %>% select( nhl_id ) %>% unlist()
player_pen  <- pen_split %>% filter( event_p1_id == this_nhl_id )
...
#   T           81.072 NJD
# Stempniak has 
# 2 rows in vs_opp_rosters_group 
#  vs_opp_rosters_C  %>% filter( last_name=="Stempniak")
# 64 rows in vs_opp_rosters, 63 as NJD, 1 as BOS 
# vs_opp_rosters  %>% filter( last_name=="STEMPNIAK")
