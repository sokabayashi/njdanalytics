...
pbp_spread <- pbp %>% 
    filter( event_type %in% event_types_study ) %>% select( -id, -xcoord, -ycoord, -event_length ) %>% distinct() %>% 
    left_join( our_games %>% select( season, game_id4, our_ha=ha ), by=c( "season", "game_id4" ) )
...
pbp_gather <- pbp_gather %>% left_join(
                stage_roster %>% select( season, session_id, game_id4, ha_number, event_player1_name=last_name ), 
                by = c( "season", "session_id", "game_id4", event_player1 = "ha_number" )
) %>% left_join(
                stage_roster %>% select( season, session_id, game_id4, ha_number, event_player2_name=last_name ), 
...
write_csv( our_F_stats, path=paste0(fig_dir, "/f_lines/njd_f_lines.csv" ) )
