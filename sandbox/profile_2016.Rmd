...
player_name <- "Alexander Burmistrov"
```
...
**Age** (on 9/15/15): `r player_bio$age_season_start`
...
### Last `r season_text` (Regular) - All Strength
...
player_recent_reg      <- player_gp %>% filter( season==recent_season_db, session_id=="2" ) %>% collect()

team_game_dates <- game_dates %>% select( season, session_id, game_date, team_short )

player_games_reg <- team_game_dates %>% 
  left_join( player_recent_reg %>% filter(
    filter_period=="all",
  filter_strength=="all",
...
p.stats <- generate_comparison_bullet_chart( this_nhl_id=player_bio$nhl_id, this_season = this_season, player_seasons, player_tbl, toi_comparison_vars, recent=TRUE, gm_requirement=10 )
