...
gp_tbl      <- tbl( nhl_db, "game_player"             ) %>% filter( session_id==this_session_id )
...
```{r}
# Find all the teams the player was on this last season.  works on one season at a time only.  
# assume player not traded back to same team in season.
# returns list 
# all game dates for player's PARENT TEAM(S) even if player did not play
# df of teams played on, one row per team
```
...
player_gp  <- gp_tbl %>% filter( nhl_id == player_bio$nhl_id, session_id==this_session_id, filter_period=="all", filter_score_diff=="all" )
...
```{r player season info}
# seasons in DB only
player_seasons_all <- player_gp %>% 
                  filter( filter_strength=="all" ) %>% 
                  group_by( season ) %>% 
                  summarize(
                    gm  = n(),
                    toi = sum(toi),
...
min_season_end    <- min( player_seasons_all$season_end, na.rm=T )
recent_season_end <- max( player_seasons_all$season_end )
...
seasons_played    <- c( player_hr$season_end, player_seasons_all$season_end ) %>% unique() %>% na.omit() %>% length()

player_gp          <- player_gp %>% filter( season >= min_season_db )
player_seasons_all <- player_seasons_all %>% filter( season >= min_season_db )
...
```{r augment team_score for this season}
## TO DO: PLAYOFFS?
team_games           <- team_score %>% filter( season==recent_season_db, session_id==this_session_id ) %>%
...
reg_season_teams <- player_gp %>% filter( filter_strength=="all" ) %>% 
...
team_y_min <- 0
y_gp_min   <- ( min( player_seasons_all$gm )*.9 ) %>% round()
y_gp_max   <- ( max( player_seasons_all$gm )*1.2 ) %>% round()
team_y_min <- y_gp_min

p.gp <- ggplot( player_seasons_all, aes(x=season_end, y=gm) )
p.gp + 
  team_lines + 
  ...
  scale_x_continuous( breaks=player_seasons_all$season_end, labels=player_seasons_all$season_age_label ) + 
...
y_lim_min_toi_gm <- min( player_seasons_all$toi_gm )*.9
y_lim_max_toi_gm <- max( player_seasons_all$toi_gm )*1.1
team_y_min <- y_lim_min_toi_gm

p.toi <- ggplot( player_seasons_all, aes(x=season_end, y=toi_gm) )
p.toi + 
  team_lines + 
...
# y_lim_min_pts_gm <- min( player_seasons_all$pts_gm ) * 0.7
y_lim_min_pts_gm <- 0
y_lim_max_pts_gm <- max( player_seasons_all$pts_gm ) * 1.2
team_y_min <- y_lim_min_pts_gm

p.pts_gm <- ggplot( player_seasons_all, aes(x=season_end, y=pts_gm) )
p.pts_gm +
...
player_recent_season      <- player_gp %>% filter( season==recent_season_db ) %>% collect()

team_game_dates <- game_dates %>% select( season, session_id, game_date, team_short )

player_games_reg <- team_game_dates %>%
  left_join( player_recent_season %>% filter( filter_strength=="all" ) %>%
      select( -game_id4, -team_short ), by = c( "season", "session_id", "game_date" ) )

player_games_reg_ev <- team_game_dates %>%
  left_join( player_recent_season %>% filter( filter_strength=="ev5on5" ) %>%
      select( -game_id4, -team_short ), by = c( "season", "session_id", "game_date" ) )

player_games_reg_pp <- team_game_dates %>%
  left_join( player_recent_season %>% filter( filter_strength=="pp" ) %>%
      select( -game_id4, -team_short ), by = c( "season", "session_id", "game_date" ) )

player_games_reg_sh <- team_game_dates %>%
  left_join( player_recent_season %>% filter( filter_strength=="sh") %>%
      select( -game_id4, -team_short ), by = c( "season", "session_id", "game_date" ) )
...
this_player <- this_player %>% left_join( player_seasons_all %>% select( season_end, season_age_label ),
                                          by="season_end" ) 
```
...
p.cf + 
  ...
  scale_x_continuous( breaks=this_player$season_end, labels=this_player$season_age_label) + 
  line_chart_theme + scale_color_few() +
...
# y_lim_min_p_60 <- min( this_player[ ,this_var] ) *.7
y_lim_min_p_60 <- 0

y_lim_max_p_60 <- max( this_player[ ,this_var]) *1.2
team_y_min <- y_lim_min_p_60

p.p60 <- ggplot( this_player, aes(x=season_end, y=p_60_ev5on5, label=round(p_60_ev5on5,2) ) )
p.p60 + 
  team_lines + 
...
p.toi <- p.toi + 
 ...
 scale_fill_manual( name=NULL,
  ...
  scale_x_continuous("Game Number", minor_breaks =NULL, breaks=seq(-0.5,100,1),  limits=c(0.5, n_games+0.5), expand = c(0, 0)) +
  theme_chances + theme(
...
# chart_filename <- paste0( fig_dir, "/", this_player$last_name, "-linemates.pdf" )
