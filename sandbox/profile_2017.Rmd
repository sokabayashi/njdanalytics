...
get_player_game_dates <- function( player_gp, recent_season_db ) {
  recent_eason_end <- get_season_end( recent_season_db )
  
  teams_game_date <- player_gp %>% filter( season==recent_season_db, filter_strength=="all" ) %>% 
  ...
  # Fix first date of first game and last game to go outsie reg season window
...
# can't believe I need to write this function...
...
# chart only last 8 seasons
num_seasons_to_chart <- 8
...
p.gp + 
  ...
  scale_x_continuous( breaks=player_seasons_all$season_end, labels=player_seasons_all$season_age_label_label) + 
...
#   scale_x_continuous( breaks=player_hr_reg$season_end, labels=player_hr_reg$season_age_label) + 
#   line_chart_theme + scale_color_few() +
...
player_recent_reg      <- player_gp %>% filter( season==recent_season_db ) %>% collect()
...
```{r cf by season data}
this_player <- player_season_all %>% filter( nhl_id == player_bio$nhl_id, season >= min_season_db, !season %in% bundled_seasons ) %>% 
