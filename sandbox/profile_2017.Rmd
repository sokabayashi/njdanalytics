...
gp_tbl      <- tbl( nhl_db, "game_player"             ) %>% filter( session_id=="2" )
...
# UPDATE: age at start of NEXT season.  This will need to be updated depending on report run date
...
# NHL seasons - count, going prior to DB data
...
game_dates_obj <- get_player_game_dates( player_gp, team_score, recent_season_db )
...
```{r career data} 
# data for top charts, across last 8 seasons
...
# grab all of player's team changes
...
p.p60 + 
  ...
  team_texts + 
  geom_text( vjust=-1, size=2 ) +
  scale_y_continuous( "", breaks=seq(0,10,0.2),  limits=c(y_lim_min_p_60,y_lim_max_p_60) ) +  
  scale_x_continuous( breaks=this_player$season_end, labels=this_player$season_age_label) + 
  line_chart_theme + scale_color_few() +
